...
- A walkthrough is pruned last, once the rest of the document has settled, because a step can name a view the prune has just dropped. A step whose stage is gone, or whose focus loses its last element, goes with it, and a tour left with fewer than two steps goes whole.
